...
const ProductDetails = ({ product: initialProduct, currentUrl: initialUrl }) => {
  ...
  const product = initialProduct || productsData.find((p) => p.id === parseInt(id));
  ...
  const originUrl = isClient ? window.location.origin : '';
  const currentUrl = initialUrl || (isClient ? `${originUrl}/propiedades/${product.id}` : '');
  ...
  return (
    // Agregamos pt-16 o pt-20 si tu Navbar es fija (fixed/sticky) para dar espacio
    <div className='flex flex-col min-h-screen bg-gray-50 w-full overflow-x-hidden pt-16 md:pt-20'>
      
      {/* Hero Image / Video Carrusel (Limitamos z-index a z-0 o z-10) */}
      <div className='w-full relative bg-black h-[40vh] md:h-[65vh] cursor-pointer overflow-hidden z-0' onClick={() => setIsFullscreen(true)}>
      ...
      <div className='w-full max-w-[1500px] mx-auto px-3 sm:px-6 lg:px-8 py-6 lg:py-10 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8'>
        ...
        <div className={`lg:col-span-4 ${isFullscreen ? 'hidden lg:block' : 'block'}`}>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden lg:sticky lg:top-24">
            <div className="h-[300px] sm:h-[380px] lg:h-[520px] w-full relative z-10 isolation-auto">
              {isClient && <MapComponent coords={coords} />}
