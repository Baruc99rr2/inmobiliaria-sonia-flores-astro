...
| 17 | **El costo de hidratación de React quedó al descubierto.** Al bajar el peso del home a la mitad (rama `fix/rendimiento-y-visuales`), el TBT medido subió de 53 ms a 116 ms de mediana, con una corrida en 316 ms. NO es una regresión de código: se midió y se descartó la sospecha inicial —el dibujo estático del canvas tarda 0,5 ms con la CPU 4× más lenta—. La causa es que el TBT solo cuenta el bloqueo **posterior al FCP**, y al llegar el FCP 2,4 s antes, el trabajo de hidratación deja de quedar escondido detrás de la espera de red. Trabajo total de hilo principal hay MENOS (17.844 ms → 11.372 ms). La tarea larga dominante son 667 ms de `client.js`, que es React hidratando `Homepage` entero con `client:load`. Sigue por debajo del umbral de 200 ms que Google considera bueno, así que no es urgente. Si algún día molesta, la salida es dejar de hidratar el home completo de una: pasar las secciones que no necesitan interactividad inmediata a `client:visible`, o directamente no hidratar las que son estáticas | `src/pages/index.astro`, `src/components/Homepage.jsx` | 🟡 Baja — **no urgente, medido y entendido** |

| 16 | **Las transformaciones de imagen de Supabase se facturan.** Desde la Parte 3 de `fix/rendimiento-y-visuales`, las fotos que sube la dueña se sirven por el endpoint `/storage/v1/render/image/public/...` con `width` y `quality`, en vez de bajarse enteras. Eso lo cobra Supabase por cantidad de imágenes de origen distintas, según el plan. Hoy son ~21, así que no preocupa; el punto es que **crece a medida que la dueña carga propiedades**. Mirar el consumo en el panel de Supabase dentro de unos meses. Si algún día molesta, la salida es pre-generar como se hizo con las legacy, o mover la conversión al momento de subir (`src/lib/admin/subir.ts` ya comprime del lado del navegador, podría además guardar dos tamaños) | `src/lib/imagen.js`, Dashboard de Supabase → Storage | 🟡 Baja — **revisar en unos meses** |
